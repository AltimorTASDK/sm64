...
static void geo_process_camera(struct GraphNodeCamera *node) {
    ...
    mtxf_rotate_xy(rollMtxf, node->rollScreen);
    ball_get_camera_transform(cameraTransform, rollMtxf, gMarioState, node);

    mtxf_to_mtx(rollMtx, rollMtxf);
...
static s32 obj_is_in_view(struct GraphNodeObject *node, Mat4 matrix) {
    ...
    halfFov = (gCurGraphNodeCamFrustum->fov / 2.0f + 1.0f) * 32768.0f / 180.0f * 4.0f / 3.0f + 0.5f;
