...
s32 mario_get_floor_class(struct MarioState *m) {
    ...
    if (m->action == ACT_CRAWLING && m->floorNormal.y > 0.5f && floorClass == SURFACE_CLASS_DEFAULT) {
        floorClass = SURFACE_CLASS_NOT_SLIPPERY;
    }
...
static void set_mario_y_vel_based_on_fspeed(struct MarioState *m, f32 initialVelY, f32 multiplier) {
    ...
    f32 speed = min(sqrtf(sqr(m->vel[0]) + sqr(m->vel[2])), 48.0f);
    m->vel[1] = initialVelY + get_additive_y_vel_for_jumps() + speed * multiplier;
...
static u32 set_mario_action_airborne(struct MarioState *m, u32 action, u32 actionArg) {
    ...
    switch (action) {
        ...
        case ACT_DIVE:
            ...
            if (forwardVel < 48.0f && (forwardVel = m->forwardVel + 15.0f) > 48.0f) {
                forwardVel = 48.0f;
            }
        ...
        case ACT_LONG_JUMP:
            ...
            if (forwardVel < 48.0f && (forwardVel = m->forwardVel * 1.5f) > 48.0f) {
                forwardVel = 48.0f;
            }
            mario_set_forward_vel(m, forwardVel);
            break;
        ...
        if ((m->doubleJumpTimer == 0) || (m->squishTimer != 0)) {
            ...
            switch (m->prevAction) {
                ...
                case ACT_DOUBLE_JUMP_LAND:
                    ...
                    } else if (sqr(m->vel[0]) + sqr(m->vel[2]) > sqr(20.0f)) {
